...
function GLACEBASE:ApplyPlayerFunctions( ply )
    ...
    if SERVER then
        ...
        -- Sends a debug message to console. Only works if glacebase_debug is set to 1
        function GLACE:DevMsg( ... )
            if !GetConVar( "glacebase_debug" ):GetBool() then return end
        ...
        function GLACE:CanShootAt( ent ) 
            local result = self:Trace( nil, ent:WorldSpaceCenter(), COLLISION_GROUP_NONE, MASK_SHOT_PORTAL )
            return result.Entity == ent
        end
        ...
        function GLACE:FindInSphere( pos, dist, filter )
            ...
            for k, v in ipairs( ents.FindInSphere( pos, dist ) ) do
                if IsValid( v ) and v != self and ( !filter or filter( v ) ) then
